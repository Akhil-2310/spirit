...
import { createWalletClient, createPublicClient, http as arkivHttp, type PublicArkivClient } from "@arkiv-network/sdk";
...
if (!PRIVATE_KEY) {
  throw new Error(
    "ARKIV_PRIVATE_KEY is required. Get one from https://arkiv.network"
  );
}
...
export const arkivPublic: PublicArkivClient = createPublicClient({
  chain: mendoza,
  transport: arkivHttp("https://mendoza.hoodi.arkiv.network/rpc"),
});
