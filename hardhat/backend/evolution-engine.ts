...
const SOUL_ADDRESS = process.env.SOUL_CONTRACT! as `0x${string}`;
...
async function fetchTxsForAddress(address: `0x${string}`): Promise<Tx[]> {
  const url =
    `${BLOCKSCOUT_BASE}/api?module=account&action=txlist` +
    `&address=${address}&startblock=0&endblock=99999999&sort=asc`;
  const res = await fetch(url);
  const json = (await res.json()) as any;
  if (json.status !== "1") return [];
  return json.result as Tx[];
}
...
function computeAttributesFromTxs(txs: Tx[], address: `0x${string}`) {
  if (txs.length === 0) {
    return {
      aggression: 10,
      serenity: 60,
  ...
  const influence = Math.round(
    100 * (0.6 * clamp01(txCount / 200) + 0.4 * clamp01(uniquePeers / 100))
  );
...
async function evolveForAddress(address: `0x${string}`) {
 ...
 */
async function getAllSpiritOwners(): Promise<`0x${string}`[]> {
