...
app.post("/api/evolve", async (req: any, res: any) => {
  try {
    ...
    await evolveForAddress(address as `0x${string}`);
...
app.post("/api/evolve/all", async (req: any, res: any) => {
  try {
...
app.get("/api/spirits/owners", async (req: any, res: any) => {
  try {
    const owners = await getAllSpiritOwners();
    res.json({
      success: true,
