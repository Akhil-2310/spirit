...
const GRAFFITI_ADDRESS = process.env.GRAFFITI_CONTRACT! as `0x${string}`;
...
async function syncGraffitiHistory(fromBlock: bigint, toBlock: bigint) {
  ...
  for (const log of logs) {
    try {
      ...
      const { x, y, tokenId, color, timestamp } = decoded.args as any;
